...
const ExamResultsPage: React.FC<ExamResultsPageProps> = ({
  ...
  const questionToDisplay: Question | undefined =
    questions[selectedQuestionIndex];
  const userAnswerDetails: string | undefined = questionToDisplay
    ? userAnswers[questionToDisplay.id]
    : undefined;
  const isCorrectDetails: boolean =
    questionToDisplay && userAnswerDetails !== undefined
      ? userAnswerDetails === questionToDisplay.correct_answer
      : false;
  ...
  return (
    <div
      className={`bg-slate-50 p-6 md:p-8 rounded-lg shadow-lg max-w-3xl mx-auto my-8 border-t-8 ${isPassed ? "border-green-500" : "border-red-500"}`}
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 text-center md:text-left">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-lg shadow-sm">
          ...
          <p
            className={`text-2xl font-semibold ${isPassed ? "text-green-600" : "text-red-600"}`}
          >
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-lg shadow-sm md:col-span-2">
          ...
          <p className="text-sm text-gray-500 mt-1">
            (Pass mark: {passMarkAbsolute} correct out of {questions.length}{" "}
            questions)
          </p>
        </div>
      </div>
          ...
          <div className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white p-4 rounded-md shadow-lg relative">
            <div className="flex justify-between items-center">
              <p className="font-semibold">
                If you found this helpful, please consider sponsoring to support
                further development!
              </p>
              <button
                onClick={(e) => {
      ...
      <div className="mb-8">
        {" "}
        {/* This div now follows the sponsor message or is at top if message hidden */}
        ...
        <div className="flex overflow-x-auto space-x-2 p-2 mb-4 bg-slate-200 rounded-md shadow-sm">
          ...
          {questions.length === 0 && (
            <p className="text-gray-500 p-2">No questions to review.</p>
          )}
        </div>
          ...
          {questionToDisplay ? (
            <>
              ...
              {userAnswerDetails !== undefined ? (
                <>
                  <p
                    className={`text-sm ${isCorrectDetails ? "text-green-700" : "text-red-700"}`}
                  >
                    Your answer:{" "}
                    <span className="font-bold">
                      {String(userAnswerDetails).toUpperCase()}.{" "}
                      {userAnswerTextDetails}
                    </span>
                    {isCorrectDetails ? (
                  ...
                  {!isCorrectDetails && (
                    <p className="text-sm text-blue-700 font-medium mt-1">
                      Correct answer:{" "}
                      <span className="font-bold">
                        {String(questionToDisplay.correct_answer).toUpperCase()}
                        . {correctAnswerTextDetails}
                      </span>
                    </p>
                  )}
                </>
              ...
              {questionToDisplay.explanation && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <h5 className="text-xs font-semibold text-gray-500 mb-1 uppercase">
                    Explanation
                  </h5>

                  <p className="text-sm text-gray-700 whitespace-pre-wrap">
                    {selectedLanguageCode !== "de" &&
                    (questionToDisplay as any).translation?.[
                      selectedLanguageCode
                    ]?.context
                      ? (questionToDisplay as any).translation[
                          selectedLanguageCode
            ...
            <p className="text-gray-500">
              Select a question number to see details.
            </p>
          )}
        </div>
      </div>
