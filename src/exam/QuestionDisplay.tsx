...
const QuestionDisplay: React.FC<QuestionDisplayProps> = ({
  ...
  if (!currentQuestion) {
    return (
      <p className="text-xl text-gray-700 p-4 text-center">
        Loading question...
      </p>
    );
  }
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-2">
        Q {currentExamQuestionIndex + 1}/{totalQuestions}
      </p>
      ...
      <div className="space-y-3 mb-6">
        {currentQuestion.options.map((opt: Option) => (
          <label
            key={opt.id}
            className={`flex flex-col items-start p-3 border rounded-md transition-all cursor-pointer hover:bg-gray-50 ${examUserAnswers[currentQuestion!.id] === opt.id ? "bg-blue-100 border-blue-400 ring-2 ring-blue-300" : "border-gray-300"}`}
          >
            <div className="flex items-center w-full">
              <input
                ...
                onChange={() =>
                  handleExamAnswerSelection(currentQuestion!.id, opt.id)
                }
