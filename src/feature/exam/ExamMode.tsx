...
const ExamMode: React.FC<ExamModeProps> = ({
  ...
  const [currentExamQuestionIndex, setCurrentExamQuestionIndex] =
    useState<number>(0);
  ...
  const currentQuestion: Question | null =
    questions && questions.length > 0
      ? questions[currentExamQuestionIndex]
      : null;
  ...
  const unansweredQuestionsCount =
    questions.length - Object.keys(examUserAnswers).length;

  const progressPercentage =
    examDuration > 0 ? (timeRemaining / examDuration) * 100 : 0;
  let progressBarColorClass;
  const percentageRemainingForColor =
    examDuration > 0
      ? (timeRemaining / examDuration) * 100
      : timeRemaining > 0
        ? 100
        : 0;
    ...
    <div className="bg-white p-4 md:p-6 rounded-lg shadow-xl max-w-3xl mx-auto">
      ...
      <div className="mt-8 flex space-x-4">
        <button
          ...
          className="md:w-2/4 border border-indigo-500 text-indigo-500 font-bold py-2 px-4 rounded hover:bg-indigo-100 transition-colors"
        >
          Cancel Exam
        </button>
        <button
