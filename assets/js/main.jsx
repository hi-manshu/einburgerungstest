...
const ExamMode = ({ questions: initialExamQuestions, onNavigateHome, examDuration = 600 }) => {
    ...
    const handleSubmitExam = useCallback((isAutoSubmit = false) => {
        ...
        // if (isAutoSubmit) { console.log("Exam auto-submitted."); } // Removed debug log
    }, []);
...
const App = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            } else if (!tempQuestions.length) {
                setLoadingError('No questions in data file.');
    ...
    const renderContent = () => {
        ...
        if (currentView === 'error') return (
            <div className="text-center text-red-600 p-6 bg-red-50 rounded-lg shadow-md">
                ...
                <p className="mt-4">Try refreshing. Ensure data files are correct.</p>
            </div>
        );
        // if (loadingError && currentView === 'home') console.warn("Non-critical errors:", loadingError); // Removed debug log
